...
public static class WalSegmentLayout
{
    ...
    public const string Extension = ".tslwal";

    /// <summary>旧版（PR #13）单文件 WAL 的文件名。</summary>
    public const string LegacyActiveFileName = "active.tslwal";

    /// <summary>
    /// 根据 startLsn 生成 segment 文件名（不含目录）：<c>{startLsn:X16}.tslwal</c>。
    /// </summary>
    /// <param name="startLsn">segment 起始 LSN。</param>
    /// <returns>segment 文件名，如 <c>0000000000000001.tslwal</c>。</returns>
    ...
    /// </summary>
    /// <param name="fileName">仅文件名部分（不含目录），如 <c>0000000000000A31.tslwal</c>。</param>
    ...
    /// <summary>
    /// 按 startLsn 升序枚举一个 wal/ 目录下的全部合法 segment（自动跳过 .tmp、legacy active.tslwal 及未知文件）。
    /// </summary>
    /// <param name="walDir">WAL 子目录路径。</param>
    ...
    /// <summary>
    /// 若 <c>wal/active.tslwal</c>（旧版单文件）存在，则读取其 <c>WalFileHeader.FirstLsn</c>，
    /// 将文件重命名为 <c>{firstLsn:X16}.tslwal</c>，完成 legacy → segmented 格式升级。
    /// 文件不存在时静默返回。
    /// </summary>
    /// <param name="walDir">WAL 子目录路径。</param>
