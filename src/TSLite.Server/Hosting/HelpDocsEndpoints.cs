    ...
    private static async Task ServeAsync(HttpContext ctx, string relativePath, string helpRoot)
    {
        ...
        if (!File.Exists(Path.Combine(helpRoot, "index.html")))
        {
            ...
            await ctx.Response.WriteAsync(
                "TSLite Help docs are not built. Run `dotnet tool restore` and `dotnet tool run jekyllnet build --source docs --destination src/TSLite.Server/wwwroot/help` before publishing."
