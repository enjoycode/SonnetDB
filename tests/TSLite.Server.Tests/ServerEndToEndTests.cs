...
public sealed class ServerEndToEndTests : IAsyncLifetime
{
    ...
    {
        _dataRoot = Path.Combine(Path.GetTempPath(), "tslite-server-tests-" + Guid.NewGuid().ToString("N"));
    ...
    public async Task Metrics_ReturnsPrometheusText_WithoutAuth()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, resp.StatusCode);
        var body = await resp.Content.ReadAsStringAsync();
        Assert.Contains("tslite_uptime_seconds", body);
        Assert.Contains("tslite_databases", body);
    }
