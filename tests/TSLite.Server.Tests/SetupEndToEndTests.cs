...
public sealed class SetupEndToEndTests : IAsyncLifetime
{
    ...
    {
        _dataRoot = Path.Combine(Path.GetTempPath(), "tslite-setup-e2e-" + Guid.NewGuid().ToString("N"));
    ...
    public async Task GetSetupStatus_WithEmptySystemDirectory_ReturnsNeedsSetup()
    {
        ...
        Assert.StartsWith("tslite-", resp.SuggestedServerId);
    ...
    {
        const string bearerToken = "tslite-admin-bootstrap-token";
        ...
        Assert.Equal("tslite-dev-01", payload!.ServerId);
        ...
        Assert.Equal("tslite-dev-01", status.ServerId);
